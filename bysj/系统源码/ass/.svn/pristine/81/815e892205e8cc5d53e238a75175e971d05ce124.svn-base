...
function downLoad(testId){
	location.href="<%=basePath%>front/downLoad/"+testId+"";
}
	
	function pageJump(currentPage) {
		location.href = "<%=basePath%>front/libraryTest/"+currentPage;
	}
	
	function allTest(){
		location.href = "<%=basePath%>front/listPageTest";
	}
</SCRIPT>
</HEAD>
<BODY>
					...
					<table class="table table-hover table-condensed">
						...
						<tbody>
							<c:forEach items="${pageInfo.data}" var="item" varStatus="status">
								...
								<td><a href='javascript:downLoad(${item.id })'>下载</a></td>
								</tr>
							</c:forEach>
						</tbody>
					</table>
