...
<body>
			...
			<table class="table table-hover table-condensed">
				...
				<tbody>
					<c:forEach var="item" items="${pageInfo.data}" varStatus="status">
						...
						<td>${item.skey1 }</td>
						<td><a
							href="javascript:querySinglesel(${item.id },'${item.skey1 }')">审核</a></td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
	...
	<script type="text/javascript">
		...
		function pageJump(currentPage) {
			location.href = "manage/tSinglesel/listTSingleselTemp/"+currentPage;
		}
		...
		encodeURI 进行加密
		 */
		function querySinglesel(id, skey1) {
			var skey = encodeURI(encodeURI(skey1));
