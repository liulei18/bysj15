...
<body>
			...
			<table class="table table-hover table-condensed">
				...
				<tbody>
					<c:forEach var="item" items="${pageInfo.data}" varStatus="status">
						...
						<td>${item.skey1 }</td>
						<td><a
							href="javascript:querySubjective(${item.id },'${item.skey1 }')">审核</a></td>
						</tr>
						</tr>
					</c:forEach>
				</tbody>
	...
	function pageJump(currentPage) {
		location.href = "manage/tSubjective/listTSubjectiveTemp/"+currentPage;
	}
		...
		encodeURI 进行加密
		 */
		function querySubjective(id, skey1) {
